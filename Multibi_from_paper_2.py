...
kahp_multiplier = np.arange(0.5, 1.5, 0.5)
...
for m, mult in enumerate(kahp_multiplier):
    ...
    #MNS: need to add in the full names of these species into array for easier plotting

    #fig = plt.subplot(3, 3)
    #plt.figure.(suptitle'Observable species', fontsize=16)

    #plot species in obs_list on Observable figure - initialization component
    ...
    #print("Precal original:" + str(end_of_precal[0:5]))

    #second part of run
    final_time = 15000
    tspan2 = np.linspace(init_time, final_time, (final_time - init_time) + 1)
    initials = copy.deepcopy(end_of_precal)
    delta_index = [str(sp) for sp in model.species].index('Delta()') #This index is 0, as Delta() is the first in model.species
    initials[delta_index] = 1e-4 / KfNcp.value  # 0.36 Leonard's number #10000 their number
    x = sim.run(tspan=tspan2, initials=initials)

    #plot species in obs_list on one figure - post-initialization component
    if plot_subplots:
        for i, obs in enumerate(obs_list):
            plt.subplot(3,3,i+1, title = obs)
            vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
            ...
            if i == len(obs_list) - 2:
                plt.xlabel('Time (s)')
        plt.show() #MNS- check indentation of this line
    ...
    #this section for plotting things individually

    # for obs in ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']:
    #     plt.figure(obs)
    #     vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
    #     plt.plot(tspan2, x.observables[obs] / 6.022e23 / vol, lw=2, label='Delta stimulus = 1e-3', color = 'm')
    # # 	if obs == 'H2np':
    # # 		plt.plot(tspan2, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')
    #     plt.legend()
    #     # plt.plot.xlabel('Time (s)')
    #     # plt.plot.ylabel(str(obs) + '(moles/L')
    # plt.show()

    # to plot the post-initialization part - all subplots on one figure
    if plot_subplots:
        for i, obs in enumerate(obs_list):
            plt.subplot(3,3,i+1, title = obs)
            vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
    ...
    #Make True if you want to plot individual species over range
    do_iteration = False
    if do_iteration:
        range1 = np.arange(0.03, 0.3, 0.03)
        plt.figure('Iterating over rNbox values')
        for q, val_1 in enumerate(range1):
            rNbox.value = val_1
            colors = sns.color_palette("Greens", len(range1)).as_hex()

            # bring the system back to kDelp of 0, like it is initially; let the system equilibrate
            tspan1 = np.linspace(0, init_time, init_time+1)
            ...
            x = sim.run(tspan=tspan1) #MS: does this need to have initials?

            obs_list_2 = ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']

            # plot species in obs_list_2 on Observable figure - initialization component
            for i, obs in enumerate(obs_list_2):
                #plt.figure(obs)
                plt.subplot(3, 3, i + 1, title=obs)
                vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
                plt.plot(tspan1, x.observables[obs]/6.022e23/vol, lw=2, color = colors[q]) #this puts the Y axis in Molar units
                if obs == 'H2np':
                    plt.plot(tspan1, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')

            # set kDelp to 1e-3 and let the system run
            tspan2 = np.linspace(init_time, final_time, (final_time - init_time) + 1)
            ...
            # plot species in obs_list_2 on Observable figure - post-initialization component
            for i, obs in enumerate(obs_list_2):
                #plt.figure(obs)
                plt.subplot(3, 3, i + 1, title=obs)
                vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
    ...
    #reset rNbox if the do_iteration
    #rNbox.value = 0.03
    #print('rNbox value: '+ str(rNbox.value))



    ##########
    # Generating the bifurcation over increasing and decreasing Delta strengths
    ##########
    final_time = 10000
    Hes1_fwd_steady = []
    Hes1_rev_steady = []
    tspan2 = np.linspace(0, final_time, final_time + 1)
    delta_index = [str(sp) for sp in model.species].index('Delta()')


    # d is kDelp

    kdelp_asc = np.arange(1e-4, 1.1e-3, 1e-4)
    kdelp_desc = np.arange(1e-3, 9.9e-5, -1e-4)

    for d, kdelp in enumerate(kdelp_asc):
        #print(d, kdelp)
        if d == 0:
            #print('d == 0')
            initials = copy.deepcopy(end_of_precal)
            ...
            initials = x.species[-1]
        #initials = x.species[-1]
        #print("Precal:" + str(end_of_precal[0:5]))
        #print("x.species[-1]:" + str(initials[0:5]))
        initials[delta_index] = kdelp / KfNcp.value  # 0.36 to 3.6 molecules/min
        x = sim.run(tspan=tspan2, initials=initials) #MNS >> check initials
        Hes1_fwd_steady.append((x.observables['Hnp'][-1] + x.observables['Hcp'][-1] )/ 6.022e23 / (Vnuc + Vcyt))

    for d, kdelp in enumerate(kdelp_desc):
        #print(d, kdelp)
        initials = x.species[-1]
        #print("x.species[-1]:" + str(initials[0:5]))
        initials[delta_index] = kdelp / KfNcp.value  # 0.36 to 3.6 molecules/min
        x = sim.run(tspan=tspan2, initials= initials) #MNS >> check initials
        Hes1_rev_steady.append((x.observables['Hnp'][-1] + x.observables['Hcp'][-1] )/ 6.022e23 / (Vnuc + Vcyt))
    #############
    ...
    ax = plt.subplot(2, len(kahp_multiplier)/2, m + 1, title= 'KaHp = {0:.2f}* 1 * 0.02535'.format(mult)) #MS - automate the dimensions of subplots based on number of them
    #fig, ax = plt.subplots() #next day: the plots print separately when this line is included
    #plt.figure('Bifurcation plot, KaHp = ' + str(mult)+ '*1*0.02535')

    plt.plot(kdelp_asc, Hes1_fwd_steady, lw=2, label= 'Increasing Delta') #MNS - fix this label
    plt.plot(kdelp_desc, Hes1_rev_steady, lw=2, label='Decreasing Delta')

    #ax.text(0.55, 0.37, 'KaHp = {0:.2f} * 0.02535'.format(mult), transform =ax.transAxes)
    # print(type(ax))
    # print(ax)
    ax.text(0.65, 0.35, 'KaHp = {0:.2f} * (1*0.02535)'.format(mult), transform=ax.transAxes)
    ...
    if bifrc_labels:
        plt.legend()
        plt.title('Hysteresis of Hes1')
        plt.xlabel('Delta signal strength (unitless) for KaHp = {0:.2f} * 1 * 0.02535'.format(mult), fontsize=9, weight='bold')
        plt.ylabel('Hes1 (mol/L)', fontsize=14, weight='bold')
