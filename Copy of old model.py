...
#This file is copy of Notch_schaffer_bifurcation
#Notch model, has do_iterate block and plots multiple bifurcation plots
#Uses rates as listed in C++ code (stochastic model)
#Uses initial concentrations as derived from numbers in C++ code (stochastic model)
...
kahp_multiplier = np.arange(0.5, 2.0, 0.5)

for m, mult in enumerate(kahp_multiplier):
    ####START OF "TWO INITIAL SIMULATIONS" SECTION####
    print('This is m: ' + str(m)+ ' This is mult: ' + str(mult))
    ...
    # bifurcation
    plt.subplot(1, 3, m + 1, title= 'KaHp = {0:.2f}* 1 * 0.02535'.format(mult))
    ...
    plt.plot(kdelp_asc, Hes1_fwd_steady, lw=2, label= 'KaHp = {0:.2f} * 1 * 0.02535'.format(mult)) #MNS - fix this label
    plt.plot(kdelp_desc, Hes1_rev_steady, lw=2, label='Decreasing Delta')
    #ax.text(0.55, 0.37, 'KaHp = {0:.2f} * 1 * 0.02535'.format(mult), transform =ax.transAxes)
    # print(type(ax))
    # print(ax)
    ...
    if bifrc_labels:
        ...
        plt.xlabel('Delta signal strength (unitless) for KaHp = {0:.2f} * 0.02535'.format(mult), fontsize=9, weight='bold')
