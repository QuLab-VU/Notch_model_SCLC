...
Initial(dNotch(Hbox=None, Rbox=MultiState(None, None)), dNotch_0)
Initial(dHes1(Hbox=MultiState(None, None, None), Rbox=MultiState(None, None)), dHes1_0)
Initial(dRBPJ(Hbox=MultiState(None, None, None), Rbox=MultiState(None, None, None)), dRBPJ_0)
...
Parameter('KaHp', 1 * 0.02535) # removed the factor of 2
Parameter('KrHp', 1)  # 0)
...
for d, kdelp in enumerate(kdelp_asc):
    ...
    if d == 0:
        print('Done')
        ##what was this supposed to be?? -MS
        initials = x.species[-1]
        ##what was this supposed to be?? - MS
    else:
        initials = x.species[-1]
    ...
    initials[delta_index] = d / KfNcp.value  # 0.36 to 3.6 molecules/min
...
for d, kdelp in enumerate(kdelp_desc):
    ...
    initials = x.species[-1]
    initials[delta_index] = d / KfNcp.value  # 0.36 to 3.6 molecules/min
    x = sim.run(tspan=tspan2, initials=end_of_precal) #MNS >> check initials
