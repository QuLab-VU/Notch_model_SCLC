...
#Notch model, has 'do_iterate' block and plots multiple bifurcation plots
#Uses rates as listed in Supplemental from Berkeley Madonna code (Schafer/Agrawal's ODE model)
#Uses initial concentrations as derived from numbers in C++/stochastic model
#working model using params from notch_stochastic_model.cpp
...
#MNS: finish this later
full_names = ['delta', 'Delta protein']
...
###NEW
#    def __init__(self, name: object, parent: object = None, dimension: object = 3, size: object = None, _export: object = True) -> object:

#Compartment('cytosol', dimension=3, size=cyto_vol, parent=ec_membrane)
# Compartment('cyto_mem', dimension=2, size= 3.14e-10)
# Compartment('cytosol', )
...
#Definition of 'delay'
# e.g. Monomer('mRBPJ', ['delay'], {'delay': ['y', 'n']})
# mRBPJ(delay = 'y') represents the EARLIER number of species, or those in the PAST

Model()
Monomer('Delta')
Monomer('mNotch', ['delay'], {'delay': ['y', 'n']})
Monomer('pNotch', ['delay'], {'delay': ['y', 'n']})
Monomer('NICD', ['rbpj', 'loc'], {'loc': ['cyt', 'nuc']})
Monomer('mRBPJ', ['delay'], {'delay': ['y', 'n']})
Monomer('pRBPJ', ['nicd', 'Rbox', 'loc', 'delay'], {'loc': ['cyt', 'nuc'], 'delay': ['y', 'n']})
Monomer('mHes1', ['delay'], {'delay': ['y', 'n']})
# Hbox refers to Hes1 binding site. Its proper name is Nbox.
Monomer('pHes1', ['pHes1', 'Hbox', 'loc', 'delay'], {'loc': ['cyt', 'nuc'], 'delay': ['y', 'n']})
...
#These numbers come from Table 2
Parameter('Delta_0', 0)
Parameter('mHes1_0', 1)  # Hcm=1;		//3.64x10^(-12)
Parameter('pHes1_cyt_0', 41)  # Hcp=35;		//1.25*10^(-10)
Parameter('pHes1_nuc_0', 130)  # Hnp=109;		//2.77*10^(-9)
Parameter('dimHes1_nuc_0', int(0.02535 * pHes1_nuc_0.value ** 2))  # 101) # H2np=101;		//2.5662*10^(-9)
Parameter('mRBPJ_0', 0)  # Rcm=1;		//1.2*10^(-12)
Parameter('pRBPJ_cyt_0', 10)  # Rcp=11;		//3.74*10^(-11)
Parameter('pRBPJ_nuc_0', 422)  # Rnp=447;		//1.13*10^(-8)
Parameter('mNotch_0', 5)  # Nm=70;		//2.51*10^(-10)	//Notch mRNA
Parameter('pNotch_0', 269)  # ??# Np=8130;	//2.95*10^(-8)	//Notch protein in the cytoplasm
...
#MS:should these values below be 0?? Same as those above??
Parameter('mHes1_0_d', 1)  # Hcm=1;		//3.64x10^(-12)
Parameter('pHes1_cyt_0_d', 41)
Parameter('mRBPJ_0_d', 0)  # Rcm=1;		//1.2*10^(-12)
Parameter('pRBPJ_cyt_0_d', 10)  # Rcp=11;		//3.74*10^(-11)
Parameter('mNotch_0_d', 5)  # Nm=70;		//2.51*10^(-10)	//Notch mRNA
Parameter('pNotch_0_d', 269)  # ??# Np=8130;	//2.95*10^(-8)	//Notch protein in the cytoplasm

Initial(Delta(), Delta_0)
Initial(mHes1(delay='n'), mHes1_0)
Initial(pHes1(pHes1=None, Hbox=None, loc='cyt', delay ='n'), pHes1_cyt_0) #MS: all proteins will be labeled with delay or no delay, but only cytosolic protein can be delay = 'y'
Initial(pHes1(pHes1=None, Hbox=None, loc='nuc', delay='n'), pHes1_nuc_0)
Initial(pHes1(pHes1=1, Hbox=None, loc='nuc', delay='n') % pHes1(pHes1=1, Hbox=None, loc='nuc', delay='n'), dimHes1_nuc_0)
Initial(mRBPJ(delay='n'), mRBPJ_0)
Initial(pRBPJ(nicd=None, Rbox=None, loc='cyt', delay='n'), pRBPJ_cyt_0)
Initial(pRBPJ(nicd=None, Rbox=None, loc='nuc', delay='n'), pRBPJ_nuc_0)
Initial(mNotch(delay='n'), mNotch_0)
Initial(pNotch(delay='n'), pNotch_0)
...
#delayed
Initial(mHes1(delay = 'y'), mHes1_0_d)
Initial(mRBPJ(delay='y'), mRBPJ_0_d)
Initial(mNotch(delay='y'), mNotch_0_d)
Initial(pHes1(pHes1=None, Hbox=None, loc='cyt', delay = 'y'), pHes1_cyt_0_d)
Initial(pRBPJ(nicd=None, Rbox=None, loc='cyt', delay='y'), pRBPJ_cyt_0_d)
Initial(pNotch(delay='y'), pNotch_0_d)
...
Observable('pHes1_cyt', pHes1(loc='cyt', delay='n'))
Expression('KdNICD', 0.00385 + (0.0014 - 0.00385) * (pHes1_cyt > 100))  # degradation NICD protein #MS: look into this
...
Parameter('Ka_r', 5) #MS: what is the source for this?
# Hes1 dimer association constant (M^-1) KaHp 1x10^9 Text
Parameter('KaHp', 1 * 0.02535) # removed the factor of 2; "correct" value should be 2*0.02535
Parameter('KrHp', 1)  # 0)
...
Rule('Hes1_dimerization', pHes1(pHes1=None, Hbox=None, loc='nuc', delay='n') + pHes1(pHes1=None, Hbox=None, loc='nuc', delay='n') |
     pHes1(pHes1=1, Hbox=None, loc='nuc', delay='n') % pHes1(pHes1=1, Hbox=None, loc='nuc', delay='n'), KaHp, KrHp)
...
Expression('testing', dNotch_pHes1(x))
...
# alt rule: Rule('mHes1_ptxa', dHes1() >> dHes1() + mHes1(delay = 'y'), RfHcm/7) #MS: should this have dHes1 in it or can we get rid of that
#no need to add location for either of the following two? MS
Rule('mHes1_ptxa', None >>  mHes1(delay='y'), RfHcm/7) #MS: does this need to be made a parameter, and should the 7 be there
Rule('mHes1_appears', mHes1(delay='y') >> mHes1(delay = 'n'),Parameter('kmH_delay', 1. / TmHc))
...
Rule('mHes1_ptla', mHes1(delay='n') >> mHes1(delay='n') + pHes1(loc='cyt', delay='y'), KtrHc) #MS: should locations be specified
Rule('pHes1_appears', pHes1(loc='cyt', delay='y') >> pHes1(pHes1=None, Hbox=None, loc='cyt', delay='n'), Parameter('kpH_delay', 1. / TpHc))
...
#new rules -e
Rule('mHes1_degradation', mHes1(delay='n') >> None, KdHcm)  # //12. Hcm --> 0  KdHcm
Rule('pHes1_cyt_monomer_degradation', pHes1(pHes1=None, Hbox=None, loc='cyt', delay = 'n') >> None,
     KdHcp)  # //18. Hcp --> 0  KdHcp; //19. Hnp --> 0  KdHnp        #two rules replacing one
Rule('pHes1_nuc_monomer_degradation', pHes1(pHes1=None, Hbox=None, loc='cyt', delay = 'n') >> None,
     KdHnp)  # //18. Hcp --> 0  KdHcp; //19. Hnp --> 0  KdHnp        #two rules replacing one
Rule('pHes1_dimer_degradation', pHes1(pHes1=ANY, Hbox=None, loc='nuc', delay='n') >> None, KdH2np)  # //20. H2np --> 0  KdH2np
#MS: in the above, is adding 'delay='n'' necessary
#MS: should the entire dimer be specificed rather than the way it is written now?
...
#new rules -g
Rule('Hes1_nuclear_transport', pHes1(pHes1=None, Hbox=None, loc='cyt', delay='n') >> pHes1(pHes1=None, Hbox=None, loc='nuc', delay='n'),
     KniHcp) #MS: no need to worry about coefficient?
...
#MS: is this redundant here
Rule('dRBPJ_bind_Hes1_dimer',
     dRBPJ(Hbox=None) + pHes1(pHes1=1, Hbox=None, loc='nuc', delay='n') % pHes1(pHes1=1, Hbox=None, loc='nuc', delay='n') |
     dRBPJ(Hbox=[2, 3]) % pHes1(pHes1=1, Hbox=2, loc='nuc', delay='n') % pHes1(pHes1=1, Hbox=3, loc='nuc', delay='n'), Kn, Kn_r)
...
Parameter('Cr', 1)# Cooperativity factor for RBP-Jk DNA binding Cr 1 Text
Parameter()#
...
alpha1 = 1
alpha2 = 4*Cr*Ka**2*Kr**2*NnpD**2*RnpD2
# alpha3 = 2*Kr*RnpD*rR
...
# Print species name and its ODE
spec = 1
print(model.species[spec])
print(list(model.odes)[spec])
