...
# MNS: I got 4.581e-16 and 6.545e-17 respectively
...
#MNS: where are these reverse rates coming from?
...
Parameter('KaHp', 2 * 0.02535)      # calculated based on nuclear volume only
Parameter('KrHp', 1)  # 0)
...
#'Strength of delta signal'
Parameter('kDelp', 10**-3)
...
###___PARAMETERS_FROM_MATLAB_CODE___###

Parameter('Cr', 1)  # Cooperativity factor for RBPJ-DNA binding
Parameter('Cnr', 1) #Cooperativity factor fro RBPJ-Hes1 DNA binding
Parameter('Cn', 1) #Cooperativity factor for Hes1-DNA binding
Parameter('Kr', 3.23*10**8)
Parameter('Ka', 1*10**8) #RBPJ-NICD association constant
Parameter('Kn', 2*10**8) #Hes1-DNA association constant

Parameter('NnpD', ) #delayed nuclear NICD concentration
Parameter('RnpD', ) #delayed nuclear RBP-Jprotein concentration
Parameter('HdnpD', ) #delayed nuclear Hes1 dimer conc

Parameter('HcmD', ) #delayed cytoplasmic Hes1 mRNA concentration
Parameter('rR', 0.2) #repression constant of RBPJ alone bound to DNA



###___EXPRESSIONS_FROM_MATLAB_CODE___###


Expression('D_placeholder', 1+2*Cr*Kr**2*(1+2*Ka*NnpD+2*Ka**2*NnpD**2)*RnpD**2+2*Kr*(RnpD+Ka*NnpD*RnpD)+3*HdnpD*Kn*
           (1+2*Cnr*Kr*RnpD*(1+Cr*Kr*RnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD+Ka*(NnpD+2*Cr*Kr*NnpD*RnpD)))+6*Cn*HdnpD**2*Kn**2*
           (1+2*Cnr*Kr*RnpD*(1+Cr*Kr*RnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD+Ka*(NnpD+2*Cr*Kr*NnpD*RnpD)))+6*Cn**2*HdnpD**3*
           Kn**3*(1+2*Cnr*Kr*RnpD*(1+Cr*Kr*RnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD+Ka*(NnpD+2*Cr*Kr*NnpD*RnpD))))
#Supplemental doc said 'Vb', which I changed to 'Vbh'.
Expression('RfHcm', 1/D_placeholder*((1+4*Cr*Ka**2*Kr**2*NnpD**2*RnpD**2+2*Kr*RnpD*rR+2*Cr*Kr**2*RnpD**2*rR**2+2*Ka*
            Kr*NnpD*RnpD*(1+2*Cr*Kr*RnpD*rR)+3*HdnpD*Kn*rNbox*(1+2*Cnr*Kr*RnpD*(Ka*NnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD+rR
            +2*Cr*Ka*Kr*NnpD*RnpD*rR+Cr*Kr*RnpD*rR**2))+6*Cn*HdnpD**2*Kn**2*rNbox**2*(1+2*Cnr*Kr*RnpD*(Ka*NnpD+2*Cr*
            Ka**2*Kr*NnpD**2*RnpD+rR+2*Cr*Ka*Kr*NnpD*RnpD*rR+Cr*Kr*RnpD*rR**2))+6*Cn**2*HdnpD**3*Kn**3*rNbox**3*
            (1+2*Cnr*Kr*RnpD*(Ka*NnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD+rR+2*Cr*Ka*Kr*NnpD*RnpD*rR+Cr*Kr*RnpD*rR**2)))
            *Vbh+2*Ka*Kr*NnpD*(1+3*Cnr*HdnpD*Kn*rNbox*(1+2*Cn*HdnpD*Kn*rNbox+2*Cn**2*HdnpD**2*Kn**2*rNbox**2))*RnpD
            *(tc+2*Cr*Kr*RnpD*(Ka*NnpD+rR*tc))*Vmaxh))

Expression('C_placeholder', 1+Kr*(3+2*Ka*NnpD)*RnpD+6*Cr*Kr**2*(1+2*Ka*NnpD+2*Ka**2*NnpD**2)*RnpD**2+6*Cr**2*Kr**3*
           (1+3*Ka*NnpD+6*Ka**2*NnpD**2+6*Ka**3*NnpD**3)*RnpD**3+3*HdnpD*Kn*(1+3*Cnr*Kr*RnpD*(1+2*Cr*Kr*RnpD+2*Cr**2*
            Kr**2*RnpD**2+12*Cr**2*Ka**3*Kr**2*NnpD**3*RnpD**2+4*Cr*Ka**2*Kr*NnpD**2*RnpD*(1+3*Cr*Kr*RnpD)+Ka*NnpD*
            (1+4*Cr*Kr*RnpD+6*Cr**2*Kr**2*RnpD**2)))+6*Cn*HdnpD**2*Kn**2*(1+3*Cnr*Kr*RnpD*(1+2*Cr*Kr*RnpD+2*Cr**2*Kr**2
            *RnpD**2+12*Cr**2*Ka**3*Kr**2*NnpD**3*RnpD**2+4*Cr*Ka**2*Kr*NnpD**2*RnpD*(1+3*Cr*Kr*RnpD)+Ka*NnpD*
            (1+4*Cr*Kr*RnpD+6*Cr**2*Kr**2*RnpD**2)))+6*Cn**2*HdnpD**3*Kn**3*(1+3*Cnr*Kr*RnpD*(1+2*Cr*Kr*RnpD+2*Cr**2*
            Kr**2*RnpD**2+12*Cr**2*Ka**3*Kr**2*NnpD**3*RnpD**2+4*Cr*Ka**2*Kr*NnpD**2*RnpD*(1+3*Cr*Kr*RnpD)+Ka*NnpD*
            (1+4*Cr*Kr*RnpD+6*Cr**2*Kr**2*RnpD**2))))
Expression('RfRcm', 1/C_placeholder*((1+36*Cr**2*Ka**3*Kr**3*NnpD**3*RnpD**3+3*Kr*RnpD*rR+6*Cr*Kr**2*RnpD**2*rR**2+6*
            Cr**2*Kr**3*RnpD**3*rR**3+12*Cr*Ka**2*Kr**2*NnpD**2*RnpD**2*(1+3*Cr*Kr*RnpD*rR)+2*Ka*Kr*NnpD*RnpD*(1+3*Cr*
            Kr*RnpD*rR)**2+3*HdnpD*Kn*rNbox*(1+3*Cnr*Kr*RnpD*(12*Cr**2*Ka**3*Kr**2*NnpD**3*RnpD**2+4*Cr*Ka**2*Kr*
            NnpD**2*RnpD*(1+3*Cr*Kr*RnpD*rR)+rR*(1+2*Cr*Kr*RnpD*rR+2*Cr**2*Kr**2*RnpD**2*rR**2)+Ka*NnpD*(1+4*Cr*Kr*RnpD
            *rR+6*Cr**2*Kr**2*RnpD**2*rR**2)))+6*Cn*HdnpD**2*Kn**2*rNbox**2*(1+3*Cnr*Kr*RnpD*(12*Cr**2*Ka**3*Kr**2*
            NnpD**3*RnpD**2+4*Cr*Ka**2*Kr*NnpD**2*RnpD*(1+3*Cr*Kr*RnpD*rR)+rR*(1+2*Cr*Kr*RnpD*rR+2*Cr**2*Kr**2*RnpD**2
            *rR**2)+Ka*NnpD*(1+4*Cr*Kr*RnpD*rR+6*Cr**2*Kr**2*RnpD**2*rR**2)))+6*Cn**2*HdnpD**3*Kn**3*rNbox**3*(1+3*Cnr*Kr
            *RnpD*(12*Cr**2*Ka**3*Kr**2*NnpD**3*RnpD**2+4*Cr*Ka**2*Kr*NnpD**2*RnpD*(1+3*Cr*Kr*RnpD*rR)+rR*(1+2*Cr*Kr*
            RnpD*rR+2*Cr**2*Kr**2*RnpD**2*rR**2)+Ka*NnpD*(1+4*Cr*Kr*RnpD*rR+6*Cr**2*Kr**2*RnpD**2*rR**2))))*Vbr+Ka*Kr*
            NnpD*RnpD*((2+9*Cnr*HdnpD*Kn*rNbox*(1+2*Cn*HdnpD*Kn*rNbox+2*Cn**2*HdnpD**2*Kn**2*rNbox**2))*tc**2+12*Cr*Kr*
            (1+3*Cnr*HdnpD*Kn*rNbox*(1+2*Cn*HdnpD*Kn*rNbox+2*Cn**2*HdnpD**2*Kn**2*rNbox**2))*RnpD*tc*(Ka*NnpD+rR*tc)+18
            *Cr**2*Kr**2*(1+3*Cnr*HdnpD*Kn*rNbox*(1+2*Cn*HdnpD*Kn*rNbox+2*Cn**2*HdnpD**2*Kn**2*rNbox**2))*RnpD**2*
            (2*Ka**2*NnpD**2+2*Ka*NnpD*rR*tc+rR**2*tc**2))*Vmaxr))

Expression('E_placeholder', 1+2*Cr*Kr**2*(1+2*Ka*NnpD+2*Ka**2*NnpD**2)*RnpD**2+2*Kr*(RnpD+Ka*NnpD*RnpD)+HdnpD*Kn*(1+2*
            Cnr*Kr*RnpD*(1+Ka*NnpD+Cr*Kr*RnpD+2*Cr*Ka*Kr*NnpD*RnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD)))
Expression('RfNm', 1/E_placeholder *(1+4*Cr*Ka**2*Kr**2*NnpD**2*RnpD**2+2*Kr*RnpD*rR+2*Cr*Kr**2*RnpD**2*rR**2+2*Ka*Kr*
            NnpD*RnpD*(1+2*Cr*Kr*RnpD*rR)+HdnpD*Kn*rNbox*(1+2*Cnr*Kr*RnpD*(Ka*NnpD+2*Cr*Ka**2*Kr*NnpD**2*RnpD+rR+2*Cr*
            Ka*Kr*NnpD*RnpD*rR+Cr*Kr*RnpD*rR**2)))*Vbn+2*Ka*Kr*NnpD*(1+Cnr*HdnpD*Kn*rNbox)*RnpD*(tc+2*Cr*Kr*RnpD*
            (Ka*NnpD+rR*tc))*Vmaxn))

###
# (d Hcm)/dt = RfHcm/7-Hcm*kdHcm
# (d Hcp)/dt = KtrHc*HcmD-kdHcp*Hcp-kniHcp*Hcp
# (d Hnp)/dt = 7*kniHcp*Hcp-kdHnp*Hnp
# Hdnp = Hnp**2*KaHp
###

# Expression('Hcm_rate', RfHcm/7-Hcm*kdHcm)
# Expression('Hcp_rate', KtrHc*HcmD-kdHcp*Hcp-kniHcp*Hcp)
# Expression('Hnp_rate', 7*kniHcp*Hcp-kdHnp*Hnp)
# Expression('Hdnp', Hnp**2*KaHp)

#
# (d Rcm)/dt = RfRcm/7-kdRcm*Rcm
# (d Rcp)/dt = KtrRc*RcmD-kdRcp*Rcp-kniRcp*Rcp
# (d Rnp)/dt = 7*kniRcp*Rcp-kdRnp*Rnp
#
# Expression('Rcm_rate', RfRcm/7-kdRcm*Rcm)
# Expression('Rcp_rate', KtrRc*RcmD-kdRcp*Rcp-kniRcp*Rcp)
# Expression('Rnp_rate', 7*kniRcp*Rcp-kdRnp*Rnp)
#
#
# (d Nm)/dt = RfNm/7-kdNm*Nm
#
# (d Np)/dt = KtrN*NmD-kdNp*Np-Np*kDelp
#
# (d Ncp)/dt = Np*kDelp-kniNcp*Ncp-kdNcp*Ncp
#
# (d Nnp)/dt = 7*kniNcp*Ncp-kdNnp*Nnp
# #
#
# Expression('Nm_rate', RfNm/7-kdNm*Nm)
# Expression('Np_rate', KtrN*NmD-kdNp*Np-Np*kDelp)
# Expression('Ncp_rate', Np*kDelp-kniNcp*Ncp-kdNcp*Ncp)
# Expression('Nnp_rate', 7*kniNcp*Ncp-kdNnp*Nnp)
...
Expression('ktrN', rNbox ** dNotch_pHes1(x) * rRbox ** dNotch_pRBPJ(x) * (
            (dNotch_pNICD(x) > 0) * tc ** (2 - dNotch_pNICD(x)) * Vmaxn + Vbn))
...
Expression('ktrR', rNbox ** dRBPJ_pHes1(x) * rRbox ** dRBPJ_pRBPJ(x) * (
            (dRBPJ_pNICD(x) > 0) * tc ** (3 - dRBPJ_pNICD(x)) * Vmaxr + Vbr))
...
Expression('ktrH', rNbox ** dHes1_pHes1(x) * rRbox ** dHes1_pRBPJ(x) * (
            (dHes1_pNICD(x) > 0) * tc ** (2 - dHes1_pNICD(x)) * Vmaxh + Vbh))
...
Rule('Notch_to_NICD', pNotch() >> NICD(rbpj=None, loc='cyt'), kDelp)
...
Rule('pHes1_monomer_degradation', pHes1(pHes1=None, Hbox=None) >> None,
     KdHes1)  # //18. Hcp --> 0  KdHcp; //19. Hnp --> 0  KdHnp
...
Rule('Hes1_nuclear_transport', pHes1(pHes1=None, Hbox=None, loc='cyt') >> pHes1(pHes1=None, Hbox=None, loc='nuc'),
     KniHcp)
...
Rule('dNotch_bind_Hes1_dimer',
     dNotch(Hbox=None) + pHes1(pHes1=1, Hbox=None, loc='nuc') % pHes1(pHes1=1, Hbox=None, loc='nuc') |
...
Rule('dHes1_bind_Hes1_dimer',
     dHes1(Hbox=None) + pHes1(pHes1=1, Hbox=None, loc='nuc') % pHes1(pHes1=1, Hbox=None, loc='nuc') |
...
Rule('dRBPJ_bind_Hes1_dimer',
     dRBPJ(Hbox=None) + pHes1(pHes1=1, Hbox=None, loc='nuc') % pHes1(pHes1=1, Hbox=None, loc='nuc') |
