...
# 	plt.figure(obs)
# 	vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt # next line puts the Y axis in Molar units
# 	plt.plot(tspan1, x.observables[obs]/6.022e23/vol, lw=2, label='pre-stimulus', color = 'g')
#     plt.legend()
    # if obs == 'H2np':
...
for obs in ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']:
	...
	plt.plot(tspan1, x.observables[obs]/6.022e23/vol, lw=5, label='pre-stimulus', color = 'g') #this puts the Y axis in Molar units
	# if obs == 'H2np':
	# 	plt.plot(tspan1, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')
	plt.legend()
...
for obs in ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']:
    plt.figure(obs)
    vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
    plt.plot(tspan2, x.observables[obs] / 6.022e23 / vol, lw=5, label='Delta stimulus = 1e-4', color = 'b') #label was obs
# 	if obs == 'H2np':
...
for obs in ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']:
	plt.figure(obs)
	vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
	plt.plot(tspan1, x.observables[obs]/6.022e23/vol, lw=5, label='pre-stimulus 2', color = 'g') #this puts the Y axis in Molar units
	if obs == 'H2np':
		plt.plot(tspan1, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')

tspan2 = np.linspace(750, 15000, (15000 - 750) + 1)
initials = x.species[-1]
delta_index = [str(sp) for sp in model.species].index('Delta()')
...
for obs in ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']:
    plt.figure(obs)
    vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
    plt.plot(tspan2, x.observables[obs] / 6.022e23 / vol, lw=5, label='Delta stimulus = 1e-3', color = 'm')
# 	if obs == 'H2np':
# 		plt.plot(tspan2, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')
    plt.legend()
    plt.title('Bifurcation of Hes1')
    plt.plot.xlabel('Time (s)')
    plt.plot.ylabel(str(obs) + '(moles/L')
plt.show()
#
...
# The crazy color section
# q=0
# for d in np.arange(0.03, 0.3, 0.03):
#     rNbox.value = d
#     colors = ['k', 'r', 'b', 'y', 'k', 'r', 'b', 'y','k', 'r', 'b', 'y',]
#     tspan1 = np.linspace(0, init_time, init_time+1)
#     delta_index = [str(sp) for sp in model.species].index('Delta()')
#     initials[delta_index] = 0 / KfNcp.value  # 0.36 Leonard's number #10000 their number
#     x = sim.run(tspan=tspan1)
...
#         plt.figure(obs)
#         vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
#         plt.plot(tspan2, x.observables[obs] / 6.022e23 / vol, lw=2, label='Number'+str(q), color = colors[q])
#     # 	if obs == 'H2np':
#     # 		plt.plot(tspan2, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')
#         plt.legend()
#         # plt.plot.xlabel('Time (s)')
...
#d is kDelp
# for d in np.arange(1e-4, 1.1e-3, 1e-4):
#     print(d)
#     initials = x.species[-1]
...
#
# for d in np.arange(1e-3, 9.9e-5, -1e-4):
#     print(d)
#     initials = x.species[-1]
#     initials[delta_index] = d / KfNcp.value  # 0.36 to 3.6 molecules/min
#     x = sim.run(tspan=tspan2, initials=initials)
