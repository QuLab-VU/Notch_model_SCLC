...
Monomer('mHes1')
# Hbox refers to Hes1 binding site. It's proper name is Nbox.
...
for obs in ['delta', 'Rcm', 'Hcm', 'Nm', 'Rcp', 'Hcp', 'Np', 'Hnp', 'H2np']:
	plt.figure(obs)
	vol = Vnuc if obs in ['Hnp', 'H2np'] else Vcyt
	plt.plot(tspan1, x.observables[obs]/6.022e23/vol, lw=5, color = 'g') #this puts the Y axis in Molar units
	if obs == 'H2np':
		plt.plot(tspan1, 0.5*KaHp.value/KrHp.value*x.observables['Hnp']**2, '--k')
